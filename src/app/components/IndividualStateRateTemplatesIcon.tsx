...
interface IndividualStateRateTemplate {
  id: string;
...
interface IndividualStateRateTemplatesIconProps {
  onLoadTemplate: (template: IndividualStateRateTemplate['template_data']) => void;
  currentSelections: Record<string, string | null>;
  currentFilterSets: FilterSet[];
  currentSelectedTableRows: { [state: string]: string[] };
  currentSelectedEntries?: { [state: string]: any[] };
}

const IndividualStateRateTemplatesIcon = ({
  onLoadTemplate,
  currentSelections,
  currentFilterSets,
  currentSelectedTableRows,
  currentSelectedEntries,
}: IndividualStateRateTemplatesIconProps) => {
  const [isOpen, setIsOpen] = useState(false);
  const [templates, setTemplates] = useState<IndividualStateRateTemplate[]>([]);
  ...
  const fetchTemplates = async () => {
    ...
    setError(null);
    try {
      const response = await fetch('/api/dashboard-templates?page=state-rate-comparison-individual');
    ...
    try {
      ...
      const response = await fetch('/api/dashboard-templates', {
        ...
        body: JSON.stringify({
          ...
          page_name: 'state-rate-comparison-individual',
        }),
      });
  ...
  const handleLoadTemplate = (template: IndividualStateRateTemplate) => {
        ...
        <div className="p-6 flex flex-col h-[80vh]">
          ...
          <div className="text-center mb-6">
            <h2 className="text-xl font-bold text-[#012C61] uppercase font-lemonMilkRegular">
              Individual State Rate Comparison Templates
            </h2>
...
export default IndividualStateRateTemplatesIcon;
