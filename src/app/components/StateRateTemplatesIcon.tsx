...
interface StateRateTemplatesIconProps {
  onLoadTemplate: (template: StateRateTemplate['template_data']) => void;
...
const StateRateTemplatesIcon = ({
  ...
  const fetchTemplates = async () => {
    ...
    setError(null);
    try {
      const response = await fetch('/api/dashboard-templates?page=state-rate-comparison-all');
    ...
    try {
      ...
      const response = await fetch('/api/dashboard-templates', {
        ...
        body: JSON.stringify({
          ...
          page_name: 'state-rate-comparison-all',
        }),
      });
  ...
  const handleLoadTemplate = (template: StateRateTemplate) => {
        ...
        <div className="p-6 flex flex-col h-[80vh]">
          ...
          <div className="text-center mb-6">
            <h2 className="text-xl font-bold text-[#012C61] uppercase font-lemonMilkRegular">
              State Rate Comparison Templates
            </h2>
...
export default StateRateTemplatesIcon;
