...
# Script to upload MEDIRATE DOCUMENTS to Vercel Blob with proper folder structure
...
find . -type f \( -name "*.pdf" -o -name "*.doc" -o -name "*.docx" -o -name "*.xls" -o -name "*.xlsx" -o -name "*.txt" \) | while read -r file; do
    # Remove leading ./ from path to maintain folder structure
    blob_path="${file#./}"
    
    echo "📤 Uploading: $blob_path"
    
    # Upload to Vercel Blob with full path
    if vercel blob put "$file" "$blob_path" --rw-token "vercel_blob_rw_4LG8E3vDGMaHJ6If_LlPD905PAzWrgVP3c9G3O7HvaqAgsy"; then
...
echo "🔗 Files are accessible via public URLs with full folder paths"
